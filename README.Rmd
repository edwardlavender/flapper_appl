...
1. `data-raw/` contains raw data for the project. 
    ...
    * `spatial/` contains spatial data for the study area:
        ...
        * `sediments/` contains a map of sediment types (named `HI1354_Sediment_Map_v2_WGS84.shp`), sourced from Howe et al. (2015) and Boroswava et al. (2018); 
...
3. `R/` contains `R` scripts that implement analyses.  
    ...
    * `examine_space_use.R` implements A2, supported by `examine_space_use_time_trials.R`;
...
3. **Implement algorithms.** Implement A1--4 via `examine_depth_use.R`, `examine_space_use.R` (and `examine_space_use_time_trials.R`), `examine_post_release_paths.R` and `examine_cooccurrences.R` respectively. 

4. **Examine results.** Examine reconstructed patterns of depth and space use, post-release movement paths and fine-scale spatial partitioning during periods of cooccurring detections for the selected individuals. 
...
Lavender et al. (in prep). A holistic framework and a new family of algorithms for reconstructing movement paths and patterns of space use from passive acoustic telemetry. 
