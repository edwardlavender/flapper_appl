...
3. `R/` contains `R` scripts that implement analyses.  
    ...
    * `examine_space_use.R` implements A2, supported by `examine_space_use_time_trials.R` and `evaluate_lcps.R`;
...
3. **Implement algorithms.** Implement A1--4 via `examine_depth_use.R`, `examine_space_use.R` (together with `examine_space_use_time_trials.R` and `evaluate_lcps.R`), `examine_post_release_paths.R` and `examine_cooccurrences.R` respectively. 
